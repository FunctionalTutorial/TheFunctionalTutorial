...
public sealed partial class MiscTab : Control
{
    ...
    public MiscTab()
    {
        ...
        //Tutorial - Begin: client language for tutorial Fluent packs
        var languageEntries = new List<OptionDropDownCVar<string>.ValueOption>();
        foreach (var cultureName in ContentLocalizationManager.SupportedCultureNames)
        {
            var locId = $"ui-options-language-{cultureName.ToLowerInvariant()}";
            var label = Loc.TryGetString(locId, out var localized)
                ? localized
                : cultureName;
            languageEntries.Add(new OptionDropDownCVar<string>.ValueOption(cultureName, label));
        }
        //Tutorial - End
        ...
        Control.AddOptionDropDown(CVars.LocCultureName, DropDownLanguage, languageEntries); //Tutorial
